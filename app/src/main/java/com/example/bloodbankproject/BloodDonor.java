...
//Menu end
        ...
        Spinner SearchbloodDon = findViewById(R.id.searchbloodDon);
        List<String> item = new ArrayList<>();
        item.add("Search By Blood Type");
        item.add("A");
        item.add("B");
        item.add("O");
        item.add("AB");
        ArrayAdapter<String> adapter = new ArrayAdapter<>(BloodDonor.this, R.layout.support_simple_spinner_dropdown_item, item);
        SearchbloodDon.setAdapter(adapter);
        SearchbloodDon.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (!"Search By Blood Type".equals(parent.getItemAtPosition(position).toString())) {
                    SearchDon = parent.getItemAtPosition(position).toString();
                    Toast.makeText(getApplicationContext(), SearchDon, Toast.LENGTH_LONG).show();
                } else {
                    SearchDon = "";
                }
    ...
    private void getData() {
        StringRequest stringRequest = new StringRequest(Request.Method.GET, "https://personable-bubble.000webhostapp.com/ViewDonor.php?blood=" + SearchDon, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        BloodDonorFetch list = new BloodDonorFetch(object.getString("Id"),
                    ...
                    adapter = new AdapterDonor(listList, getApplicationContext());
